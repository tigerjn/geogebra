...
public class SpreadsheetTableW implements /* FocusListener, */SpreadsheetTable {
	...
	/** Cell renderer */
	SpreadsheetCellRendererW defaultTableCellRenderer;
	 ...
	 * Construct table
	 */
	public SpreadsheetTableW(SpreadsheetViewW view, SpreadsheetTableModel tableModel) {
		...
		defaultTableCellRenderer = new SpreadsheetCellRendererW(app, view, this,
		...
		selectionType = SpreadsheetTableInterface.CELL_SELECT;

		// add table model listener
		((SpreadsheetTableModelSimple) tableModel)
		        .setChangeListener(new SpreadsheetTableModelListener());
	...
	private void createFloatingElements() {
		...
		blueDot.setPixelSize(SpreadsheetTableW.DOT_SIZE, SpreadsheetTableW.DOT_SIZE);
	...
	public class SpreadsheetTableModelListener implements
	...
	public void setSelectionType(int selType) {
		...
		if (view.isColumnSelect()) {
			this.selectionType = SpreadsheetTableInterface.COLUMN_SELECT;
		} else {
			...
			if (w >= 0 && h >= 0) {
				...
				if (showDragHandle) {
					gridPanel.setWidgetPosition(blueDot,
							x2 - ssLeft - SpreadsheetTableW.DOT_SIZE / 2 - 1,
							y2 - ssTop - SpreadsheetTableW.DOT_SIZE / 2 - 1);
				}
